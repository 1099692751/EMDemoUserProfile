...
//    }
    ...
    // 头像和昵称
    NSString *nickName = [SCUserProfileEntity getNickNameWithUsername:message.from];
    if (nickName) {
        model.nickname = nickName;
    } else {
        [SCUserProfileEntity saveUserProfileWithUsername:message.from forNickName:@"王" avatarURLPath:nil];
        model.nickname = [SCUserProfileEntity getNickNameWithUsername:message.from];
        nickName = model.nickname;
    }
    
    NSString *avatarURLPath = [SCUserProfileEntity getavatarURLPathWithUsername:message.from];
    if (avatarURLPath) {
        model.avatarURLPath = avatarURLPath;
    } else {
        [SCUserProfileEntity saveUserProfileWithUsername:message.from forNickName:nickName avatarURLPath:@"http://pic.58pic.com/58pic/13/04/04/61M58PICdHP.jpg"];
        model.avatarURLPath = [SCUserProfileEntity getavatarURLPathWithUsername:message.from];
    }
...
- (void)backAction
{
    if (self.deleteConversationIfNull) {
        ...
        if (message == nil) {
            ...
            // 删除会话时删除本地头像昵称
            [SCUserProfileEntity removeUserProfileWithUsername:self.conversation.chatter];
        }
    }
