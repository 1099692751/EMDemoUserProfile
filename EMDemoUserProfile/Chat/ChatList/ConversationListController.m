...
- (void)removeEmptyConversationsFromDB
{
    ...
    if (needRemoveConversations && needRemoveConversations.count > 0) {
        ...
        for (EMConversation *conversation in needRemoveConversations) {
            // 删除会话时删除本地头像昵称
            [SCUserProfileEntity removeUserProfileWithUsername:conversation.chatter];
        }
    }
}
...
//        }
        ...
        // 头像和昵称
        //从本地获取
        NSString *nickName = [SCUserProfileEntity getNickNameWithUsername:conversation.chatter];
        if (nickName) {
            model.title = nickName;
        } else {
            //本地拿不到，先在这从服务器获取昵称，再保存到本地，比如下面这个“王”就是从服务器获取了后传进来的
            [SCUserProfileEntity saveUserProfileWithUsername:conversation.chatter forNickName:@"王" avatarURLPath:nil];
            //保存到本地后，从本地获取
            model.title = [SCUserProfileEntity getNickNameWithUsername:conversation.chatter];
            nickName = model.title;
        }
        
        NSString *avatarURLPath = [SCUserProfileEntity getavatarURLPathWithUsername:conversation.chatter];
        if (avatarURLPath) {
            model.avatarURLPath = avatarURLPath;
        } else {
            [SCUserProfileEntity saveUserProfileWithUsername:conversation.chatter forNickName:nickName avatarURLPath:@"http://pic.58pic.com/58pic/13/04/04/61M58PICdHP.jpg"];
            model.avatarURLPath = [SCUserProfileEntity getavatarURLPathWithUsername:conversation.chatter];
        }
